...
class Continuous_MountainCarEnvCustom(gym.Env):
    """
    ## Description
    ...
    * v0: Initial versions release
    ...
    def _height(self, xs):
        return (np.sin(3*xs)*0.55 + 0.5 * np.cos(5*xs) * np.sin(1*xs) + 1)*0.7
    ...
    def render(self):
        ...
        extra_margin = 0.2 * world_width  # 20% запаса для отступа

        # Увеличиваем диапазон, добавляя запасы по бокам
        world_width_with_margin = world_width + 2 * extra_margin

        # Масштабируем так, чтобы весь мир помещался на экране
        scale = self.screen_width / world_width_with_margin
        carwidth = 40
        carheight = 20
